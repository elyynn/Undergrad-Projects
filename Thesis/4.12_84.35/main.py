...
# import data
# import model
# from model import train_snn, print_results, SNN_sample
# 
# # === Logging setup ===
...
#     handlers=[
#         logging.FileHandler("../result/log.txt", mode='w'),
#         logging.StreamHandler()
#     ]
# )
# 
# # === CSV init ===
# dftrain = pd.DataFrame(columns=['epoch','training accuracy'])
# dftrain.to_csv("../result/train_acc.csv", index=False)
# 
# dftest = pd.DataFrame(columns=['epoch','train Loss','test accuracy'])
# dftest.to_csv("../result/test_acc.csv", index=False)
# 
# # === Argument Parser ===
# parser = argparse.ArgumentParser()
# parser.add_argument('--mat_file', type=str, help='Path to a MAT file')
# parser.add_argument("--dataset", type=str, default="TIDIGITS")
# parser.add_argument("--train_batch_size", type=int, default=44)
...
# args.dataset_path = "../data/combined_data.mat"
# 
# # === Device Selection ===
# device = torch.device("cuda:{}".format(args.gpu_id) if args.gpu_id != -1 else "cpu")
# 
# # === Data Loading ===
...
# 
# # === Model Setup ===
# snn = SNN_sample(device=device, n_input=args.n_input, n_class=args.n_classes).to(device)
# 
# # === Training ===
...
# logging.info("Training complete. Results saved to ../result/")
...
parser.add_argument("--max_time",type=int,default=150)          # 时间维度
# Time Dimension:
