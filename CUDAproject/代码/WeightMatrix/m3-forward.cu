...
#define MASK_SIZE 5000
// Declare constant memory for the mask
__constant__ float const_mask[MASK_SIZE];  // Replace MAX_SIZE with a reasonable upper limit for mask size.
                                     ...
                                     int numCRows, int numCColumns)
{
    ...
    // Iterate through the tiles of A and B
    for (int tileId = 0; tileId < (numAColumns - 1) / TILE_WIDTH + 1; tileId++) {
        ...
        if (row < numCRows && col < numCColumns) {
            for (int i = 0; i < TILE_WIDTH; i++) {
                int aIndex = row * numAColumns + (tileId * TILE_WIDTH + i);
                float aVal = (aIndex < numARows * numAColumns) ? const_mask[aIndex] : 0.0f;
                val += aVal * tileB[i][tx];
            }
        }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // float temp_mask[MASK_SIZE] = {0};  // Fill unused space with zeros
    // cudaMemcpyToSymbol(const_mask, temp_mask, MASK_SIZE * sizeof(float));
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    matrixMultiplyShared<<<GridDim2, BlockDim2>>>(
        unrolled_matrix, matmul_output, Map_out, Height_unrolled, Height_unrolled, Width_unrolled, Map_out, Width_unrolled);   
