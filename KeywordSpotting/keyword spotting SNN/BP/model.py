...
class Swish(nn.Module):
    def __init__(self):
        ...
        # When super(Swish, self).__init__() is called, it invokes 
        # the __init__ method of nn.Module. This ensures that any necessary initialization 
        # in nn.Module is performed for the Swish instance.
...
class LIF_neurons_vias_time(nn.Module): # torch.nn, module is a class
    def __init__(self, n_neurons, max_time=MAX_TIME, act_fun=act_fun_default, device=torch.device("cpu")):
        super(LIF_neurons_vias_time, self).__init__() # In Python, super() is a built-in function that allows you to call methods from a parent
    # using super().x(), you can indeed call the method x from the parent class 
...
class SNN_sample(nn.Module):
    def __init__(self, device='cpu', n_input=620, n_class=11):
        ...
        self.net = nn.Sequential(
            ...
            Swish(),
            self.h2,
           # swish: Introduces non-linearity, helping the network to learn more complex patterns from the transformed input.
                   )
