...
{
	 int palette_idx = color/2;
	int j = color%2;

	if (j == 0){
		hdmi_ctrl->PALETTE[palette_idx] = (red << 9);
		hdmi_ctrl->PALETTE[palette_idx] |= (green << 5);
		hdmi_ctrl->PALETTE[palette_idx] |= (blue << 1);
	}
	else{
		hdmi_ctrl->PALETTE[palette_idx] |= (red << 21);
		hdmi_ctrl->PALETTE[palette_idx] |= (green << 17);
		hdmi_ctrl->PALETTE[palette_idx] |= (blue << 13);
	}
